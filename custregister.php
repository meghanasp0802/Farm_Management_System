...
if(isset($_POST['Submit'])){
   ...
   $usernamequery = " select * from custlogin where name= '$username'";
   ...
   $usercount = mysqli_num_rows($query);
    if($usercount>0){
        ...
        if($passwords == $cpasswords ){
            $insertquery = "insert into custlogin( name, password, cpassword) values('$username','$pass','$cpass') ";
            $iquery = mysqli_query($con, $insertquery);
            if($iquery){
                ?>
        <script>
            alert("Registered successfully");
            location.replace("custlogin.php");
            </script>
            <?php
   }else{
    ?>
